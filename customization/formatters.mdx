...
For formatters that produce structured output (such as the combined status-and-date shown next to the last channel message), a `TypedFormatter<I, O>` variant is available that returns a typed `O` rather than `CharSequence`.
